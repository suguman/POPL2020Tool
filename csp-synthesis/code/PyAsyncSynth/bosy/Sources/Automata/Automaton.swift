...
public enum LTL2AutomatonConverter: String {
    ...
    // system is true if synthesis is from the System's point of view (vs. Environment); if not, inputs and outputs must be flipped before closing off for asynchronous synthesis
    public func convert(ltl: String, system: Bool) -> CoBüchiAutomaton? {
        switch self {
        case .ltl3ba:
        ...
        case .asynch:
            return _asynch(ltl: ltl, system: system)
        }
    }

    public func read_automaton(automatonFile: String) ->  CoBüchiAutomaton? {
        guard let fs = try? String(contentsOfFile: automatonFile, encoding: String.Encoding.utf8) else {
            print("Automaton file " + automatonFile + " could not be read");
            exit(1)
        }

        return parseSpinNeverClaim(neverClaim: fs)
    }
    
    public static let allValues: [LTL2AutomatonConverter] = [.ltl3ba, .spot, .asynch]
}
...
func parseSpinNeverClaim(neverClaim: String) -> CoBüchiAutomaton? {
    ...
    for var line in neverClaim.components(separatedBy: "\n") {
        ...
        if line.hasPrefix("never") || line.hasPrefix("}") || line.hasPrefix("if") || line.hasPrefix("fi") {
            ...
            //print("parsing [" + components[0] + "], output ", guardStatement)
...
func _asynch(ltl: String, system: Bool) -> CoBüchiAutomaton? {
    // call asynchronous closure tool to prepare specification for synchronous synthesis

    let task = Process()
    task.launchPath = "./Tools/asynch"
    if system {
        task.arguments = ["--formula", "\"(\(ltl))\"", "--ltlconverter", "./Tools/ltl3ba", "--spinout"]
    } else {
        task.arguments = ["--formula", "\"(\(ltl))\"", "--ltlconverter", "./Tools/ltl3ba", "--spinout", "--flip"]
    }
