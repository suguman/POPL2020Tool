...
public struct SynthesisSpecification {
    ...
    public var dualized: SynthesisSpecification {
        ...
        return SynthesisSpecification(semantics: semantics.swapped, inputs: outputs, outputs: inputs, assumptions: [], guarantees: [dualizedLTL], automaton: automaton)
    }
    ...
    public static func fromJson(string: String, specDir: String) -> SynthesisSpecification? {
        ...
        // Decode automaton
        guard let autJSON = specDictionary["automaton"] else {
            Logger.default().error("no automaton given")
            return nil
        }
        guard let autFile = autJSON as? String else {
            Logger.default().error("automaton entry is not given as string")
            return nil
        }
        // fill in path for autFile
        let aut = specDir + "/" + autFile
        ...
        return SynthesisSpecification(semantics: semantics, inputs: inputs, outputs: outputs, assumptions: parsedAssumptions, guarantees: parsedGuarantees, automaton: aut)
    }
    ...
    public static func from(tlsf: String) -> SynthesisSpecification? {
        ...
        return .fromJson(string: bosyJson, specDir: "tlsf-unknown")
    }
