...
// find directory for specification file
var specDir: String
if options.specificationFile != nil {
    let specURL = URL(fileURLWithPath: options.specificationFile!)
    let specDirURL = specURL.deletingLastPathComponent()
    specDir = specDirURL.path
} else {
    specDir = FileManager.default.currentDirectoryPath
}

switch fileFormat {
case .bosy:
    guard let s = SynthesisSpecification.fromJson(string: json, specDir: specDir) else {
...
private func buildAutomaton(player: Player) -> CoBüchiAutomaton? {
    ...
    if options.monolithic || player == .environment || specification.assumptions.count > 0 || specification.automaton != "" {
        ...
        guard let automaton =  // read SPIN automaton directly if specified, otherwise convert from LTL 
                (specification.automaton != ""
                   ? options.converter.read_automaton(automatonFile: specification.automaton)
                   : options.converter.convert(ltl: ltlSpec, system: (player == .system))
                ) else {
        ...
        return automaton
    } else {                                                     //  [NOTE] This case not used in asynch. experiments
        ...
        for guarantee in specification.guarantees {
            let automatonTimer = options.statistics?.startTimer(phase: .ltl2automaton)
            guard let automaton = options.converter.convert(ltl: "!(\(guarantee))", system: (player == .system)) else {
                Logger.default().error("could not construct automaton")
                return nil
            }
            automatonTimer?.stop()
